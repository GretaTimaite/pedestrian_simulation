...
Voronoi diagrams, Steffen and Seyfried (2010) argue, can help to overcome some of the limitations of classical density by including, for instance, fractional counts (i.e. counting in an agent whose body (or a cell) intersects with an area of measure but is not fully within it). Hence, it is considered a more precise method (zhang_etal2011) and is used to pedestrian dynamics at the bottlenecks (liao_etal2014) or T-junctions and straight corridors (zhang_etal2011). 


Thus, it is not surprising that Voronoi diagram shows higher densities in all measured areas (also see the ["Voronoi" chapter](https://gretatimaite.github.io/gcs_jps/voronoi.html#discussion) in the book). 
