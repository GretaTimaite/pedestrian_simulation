...
To evaluate the accuracy of the JPS model, the following methods for measuring pedestrian dynamics were explored (also see Steffen and Seyfried [-@steffenMethodsMeasuringPedestrian2010]: 

1. Flow which is a count of heads that pass through a line, such as doors;
2. Density (including Voronoi diagram, a method partitioning region, so that each agent has an associated cell) which is measured by counting heads per area;
